...
function getSpotifyPlayButtonForPlaylist(title, playlist) {
  var embed = '<iframe src="https://embed.spotify.com/?uri=spotify:track:5JunxkcjfCYcY7xJ29tLai" frameborder="0" allowtransparency="true"></iframe>';
  var tids = [];
...
// Create our text list
function addToTextList(playList) {
		var playlistDiv = $(".playlist");
    playlistDiv.append("Hello");
	  playList.forEach(function(song) {
      console.log("!r " + song.artist_name + " " + song.title);
      //info.append("!r " + song.artist_name + " " + song.title);
    });
}
    ...
    .done(function(data) {
      ...
      if (!('songs' in data.response)) {
       ...
       //  $("#all_results").append(player);
        //});
        addToTextList(data.response.songs)
      }
    })
    .error(function() {
      info("Whoops, had some trouble getting that playlist");
